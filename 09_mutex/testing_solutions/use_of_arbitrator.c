...
/* ceiling 함수를 사용하기 위한 라이브러리
 * gcc 컴파일 시 math 라이브러리 추가할 것(옵션: -lm)
 */
#include <math.h>
...
/* Testing을 위한 전역변수들 */
int eat_count[N]; // 주어진 단위 시간 동안 식사를 대접받은 횟수 카운터
int eat_count_unit[N]; // 총 대접받은 횟수 카운터
...
/* 시간 단위 식사 횟수와 총 식사 횟수를 출력하기 위한 함수 */
...
void *philosopher(void *_name) {
    ...
    do {
        ...
        /* 식사를 할 때마다 카운터 증가
         * update_state(phil, EATING); 호출 하단에 위치
         */
        eat_count[phil]++;
        eat_count_unit[phil]++;
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        sem_init(&forks[i], 0, 1); // Assign semaphore as a binary value (0 or 1) (== mutex)
        state[i] = 0;
    }
